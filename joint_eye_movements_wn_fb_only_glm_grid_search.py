...
from new_style_fit_multimovie_ct_glm import new_style_wn_regularized_precompute_convs_and_fit_fb_only_glm
from optimization_encoder.ct_glm import fused_bernoulli_neg_ll_loss
from optimization_encoder.multimovie_ct_glm import MM_FBOnly_PreappliedSpatialSingleCellGLMLoss
from optimization_encoder.separable_trial_glm import ProxSolverParameterGenerator
from optimization_encoder.trial_glm import FittedFBOnlyGLM
...
def evaluate_eye_movement_fb_only_glm_model_fit(
    ...
    evaluation_model = MM_FBOnly_PreappliedSpatialSingleCellGLMLoss(
        timecourse_td,
        feedback_td,
        bias,
        loss_callable,
    ...
    with torch.no_grad():
        loss_total = evaluation_model(precomputed_spatial_convolutions,
...
if __name__ == '__main__':
    ...
    for idx, cell_id in enumerate(relevant_ref_cell_ids):
        ...
        for l1_ix, l1_hyperparam in enumerate(L1_GRID):
            ...
            loss, glm_fit_params = new_style_wn_regularized_precompute_convs_and_fit_fb_only_glm(
            ...
            spat_filters, timecourse_td, feedback_td = compute_spat_timecourse_feedback_filters(
                spat_w,
                spat_spline_basis.T,
                timecourse_w,
                timecourse_basis,
            ...
            mean_test_loss = evaluate_eye_movement_fb_only_glm_model_fit(
                test_movie_blocks,
                image_rescale_lambda_torch,
                (cell_type, cell_id),
                cells_ordered,
            ...
            model_summary_parameters = FittedFBOnlyGLM(cell_id, spat_w, bias, timecourse_w, feedback_w,
