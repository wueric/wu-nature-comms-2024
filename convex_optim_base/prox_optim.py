...
from convex_optim_base.optim_base import SingleProxProblem, BatchParallelProxProblem
...
def _single_proj_backtrack_search(single_prox_prob: SingleProxProblem,
    ...
    def eval_quadratic_approximation(next_vars: torch.Tensor,
        ...
        return s_loss + torch.sum(gradients_vectorized * diff) + div_mul * torch.sum(diff * diff)
...
def single_fista_solve(single_prox_prob: SingleProxProblem,
    ...
    for iter in range(max_iter):
        ...
        if verbose:
            print('iter={0}, loss={1}\r'.format(iter, descent_loss, step_size), end='')
