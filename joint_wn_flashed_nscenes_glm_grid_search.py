...
from new_style_optim_encoder.separable_trial_glm import new_style_joint_wn_flashed_ns_alternating_optim, EvalFlashedGLM
...
from optimization_encoder.trial_glm import FittedGLM, mean_bin_batch_bernoulli_spiking_neg_ll_loss, \
    make_batch_mean_binomial_spiking_neg_ll_loss, batch_poisson_spiking_neg_ll_loss
...
L21_GRID = np.logspace(np.log10(1e-5), np.log10(1e-2), 13)
#L21_GRID = np.array([1e-2, ])
...
if __name__ == '__main__':
    ...
    bump_basis_coupling_torch = torch.tensor(coupling_basis, dtype=torch.float32, device=device)
    ...
    for idx, cell_id in enumerate(relevant_ref_cell_ids):
        ...
        test_coupl_conv = flashed_ns_bin_spikes_precompute_coupling_convs2(
            nscenes_test_patch_list,
            coupling_basis,
            coupled_cells_subset,
            cells_ordered,
        ...
        for ix, grid_params in enumerated_product(L1_GRID, L21_GRID):
            ...
            wn_regularized_params = new_style_joint_wn_flashed_ns_alternating_optim(
                ...
                l21_group_sparse_lambda=l21_hyperparam,
                l1_spat_sparse_lambda=l1_hyperparam,
                initial_guess_timecourse=initial_timevector_guess_torch[None, :],
                outer_opt_verbose=True,
            )

            train_loss, (spat_w, timecourse_w, coupling_w, feedback_w, bias) = wn_regularized_params

            train_loss_matrix[idx, l1_ix, l21_ix] = train_loss
            ...
            spat_filter, time_filters, feedback_td, coupling_td = compute_full_filters(
                spat_w_np,
                spat_spline_basis.T,
                timecourse_w_np,
                timecourse_basis,
            ...
            test_loss_eval_mod = EvalFlashedGLM(
                spat_w_np,
                timecourse_w_np,
                feedback_w_np,
                coupling_w_np,
            ...
            with torch.no_grad(), autocast('cuda'):
                mean_test_loss = test_loss_eval_mod(test_spat_basis_filt,
                                                    ...
                                                    test_fb_conv,
                                                    test_coupl_conv).item()
                test_loss_matrix[idx, l1_ix, l21_ix] = mean_test_loss

            print(
                f'cell_id {cell_id}, L1 {l1_hyperparam}, L21 {l21_hyperparam}: opt_loss {train_loss}, ns_test_loss {mean_test_loss}')
        ...
        del test_spat_basis_filt, ns_feedback_spikes_torch, ns_fb_conv, ns_coupl_conv, ns_stim_time_conv
        del test_feedback_spikes_torch, test_coupl_conv, test_fb_conv
