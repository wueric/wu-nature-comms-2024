...
from dejitter_recons.estimate_image import compute_ground_truth_eye_movements, \
    estimate_frame_rate_lnp_with_fixed_eye_movements, noreduce_nomask_batch_per_bin_poisson_neg_log_likelihood
...
def batch_frame_rate_LNP_generate_known_eye_movement_trajectory_reconstructions(
    ...
    output_image_buffer = np.zeros((len(frame_rate_jitter_dataset), height, width), dtype=np.float32)
    ground_truth_buffer = np.zeros((len(frame_rate_jitter_dataset), height, width), dtype=np.float32)

    if not use_exact_eye_movements and predetermined_eye_movements is not None:
        assert predetermined_eye_movements.shape[0] == len(frame_rate_jitter_dataset), \
            'externally set eye movements must match the provided dataset'

    eye_movements_buffer = []

    pbar = tqdm.tqdm(total=len(frame_rate_jitter_dataset))
    for low in range(0, len(frame_rate_jitter_dataset), batch_size):
        high = min(len(frame_rate_jitter_dataset), low + batch_size)

        history_frames, target_frames, binned_spikes = frame_rate_jitter_dataset[low:high]
        ...
        n_total_frames = history_frames.shape[0] + target_frames.shape[0]
        ...
        estim_image = estimate_frame_rate_lnp_with_fixed_eye_movements(
            ...
            valid_region_mask,
            eye_movements,
            np.ones((high - low, binned_spikes.shape[-1]), dtype=np.float32),
            loss_fn,
            basic_rho_sched,
            prior_weight_lambda,
            adam_solver_gen,
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser("Generate reconstructions for the joint estimation of eye movements and image using the LNP model and dCNN prior")
    ...
    ground_truth, reconstructions, assumed_eye_movements = \
        batch_frame_rate_LNP_generate_known_eye_movement_trajectory_reconstructions(
            packed_lnp_tensors,
            jitter_dataloader,
            args.prior_lambda,
            noreduce_nomask_batch_per_bin_poisson_neg_log_likelihood,
