...
from new_style_fit_multimovie_ct_glm import new_style_joint_flashed_jittered_precompute_convs_and_fit_glm
...
EverythingData = namedtuple('EverythingData',
                            ['cell_id', 'training_movie_blocks', 'training_wn_movie_blocks', 'training_flashed_blocks',
...
def get_data_from_disk(out_queue: mp.Queue,
    ...
    for idx, cell_id in enumerate(relevant_cell_ids):
        ...
        out_queue.put(EverythingData(cell_id, training_movie_blocks, training_wn_movie_blocks, training_flashed_blocks,
...
def fit_everything_model_from_queue(input_data_queue: mp.Queue,
    ...
    while True:
        ...
        if not input_data_queue.empty():
            ...
            next_data_chunk = input_data_queue.get(block=True)  # type: Union[str, EverythingData]
            ...
            loss, glm_fit_params = new_style_joint_flashed_jittered_precompute_convs_and_fit_glm(
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Fit LNBRC (coupled) encoding model jointly to eye movements, flashed images, and white noise stimuli. NOT USED IN PAPER.')
    ...
    parser.add_argument('-g', '--l21_couple', type=float, default=1e-5,
                        help='override hyperparameter for L21 group sparsity for coupling filters')
