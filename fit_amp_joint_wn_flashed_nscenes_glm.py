...
from glm_precompute.flashed_glm_precompute import flashed_ns_precompute_spatial_basis, \
    flashed_ns_bin_spikes_precompute_feedback_convs2, flashed_ns_bin_spikes_precompute_coupling_convs2, \
...
from glm_precompute.wn_glm_precompute import preapply_spatial_basis_to_wn_and_temporally_upsample, \
    ...
    wn_bin_spikes_precompute_coupling_convs
...
from new_style_optim_encoder.separable_trial_glm import new_style_joint_wn_flashed_ns_alternating_optim
...
from optimization_encoder.trial_glm import FittedGLM, FittedGLMFamily, \
    mean_bin_batch_bernoulli_spiking_neg_ll_loss, make_batch_mean_binomial_spiking_neg_ll_loss, \
    batch_poisson_spiking_neg_ll_loss

if __name__ == '__main__':

    parser = argparse.ArgumentParser('Fit LNBRC (coupled) encoding model jointly to flashed and white noise stimuli')
    ...
    for idx, cell_id in enumerate(relevant_cell_ids):
        ...
        coupled_cells_list = [cell_id, *all_coupled_cell_ids_ordered]
        coupled_cells_listlist = [[x, ] for x in coupled_cells_list]
        print(f'n_coupled_cells {len(coupled_cells_listlist) - 1}')

        ### Get cropping objects ########################################
        crop_bbox = bounding_boxes_by_type[cell_type][idx]
        ...
        wn_coupling_basis_applied = wn_bin_spikes_precompute_coupling_convs(
            training_wn_movie_block,
            wn_timebin,
        ...
        ns_coupl_conv = flashed_ns_bin_spikes_precompute_coupling_convs2(
            nscenes_patch_list,
            coupling_basis,
            coupled_cells_subset,
            cells_ordered,
        ...
        ns_feedback_start_sample = feedback_basis.shape[1] - 1
        wn_feedback_start_sample = feedback_basis.shape[1] - 1
        ...
        if args.debug:
            solver_schedule = (ProxSolverParameterGenerator(500, 250), ProxSolverParameterGenerator(250, 250))
        else:
            solver_schedule = (ProxSolverParameterGenerator(1000, 500), ProxSolverParameterGenerator(1000, 500))

        if args.stupid:
            solver_schedule = (ProxSolverParameterGenerator(50, 25), ProxSolverParameterGenerator(50, 25))

        torch.cuda.synchronize(device)

        wn_regularized_params = new_style_joint_wn_flashed_ns_alternating_optim(
            ns_spat_basis_filt,
        ...
        coupling_td = coupling_w_np @ coupling_basis
        ...
        model_summary_parameters = FittedGLM(cell_id, spat_filt_as_image_shape, bias_np, timecourse_w_np, feedback_w_np,
                                             (coupling_w_np, np.array(all_coupled_cell_ids_ordered)),
                                             fitting_params_to_store, train_loss, None)

        fitted_models_dict[cell_id] = model_summary_parameters

        model_fit_pbar.update(1)

        del spat_filt, timecourse_w, coupling_w, feedback_w, bias

        # GPU cleanup
        del wn_center_spikes_torch
        del wn_feedback_basis_applied, wn_coupling_basis_applied
        del wn_movie_upsampled_flat_torch
        del ns_feedback_spikes_torch, ns_fb_conv, ns_coupl_conv, ns_stim_time_conv
